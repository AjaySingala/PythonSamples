...
schema = avro.schema.parse(open("measure_array.avsc").read())

# Creation of DataFileWriter instance with above schema
writer = DataFileWriter(
    open("measure_array.avro", "wb"), DatumWriter(), schema)
...
reader = DataFileReader(open("measure_array.avro", "rb"), DatumReader())
for pair in reader:
    print(pair)
